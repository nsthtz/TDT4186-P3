...
public class Process {
	...
	public Event cpuActive(long period, long clock) {

		// Process has enough time to finish, queue finalization.
		if ((cpuTimeNeeded < period) && (cpuTimeNeeded < timeToNextIoOperation)) {
			timeSpentInCpu += cpuTimeNeeded;
			return new Event(Event.END_PROCESS, clock + cpuTimeNeeded);
		}

		if (timeToNextIoOperation < period) {
			long nextIo = timeToNextIoOperation;
			timeSpentInCpu += timeToNextIoOperation;
			cpuTimeNeeded -= timeToNextIoOperation;
			timeToNextIoOperation = avgIoInterval;
			return new Event(Event.IO_REQUEST, clock + nextIo);
		}

		cpuTimeNeeded -= period;
		timeSpentInCpu += period;
		timeToNextIoOperation -= period;
		return new Event(Event.SWITCH_PROCESS, clock + period);

	}

	public Event ioActive(long ioTime, long clock) {
	...
	public void readyQueued() { nofTimesInReadyQueue++; }
