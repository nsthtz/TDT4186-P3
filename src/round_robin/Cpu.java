...
public class Cpu {
    ...
    public Event insertProcess(Process p, long clock) {
        // Add to queue, update stat
        cpuQueue.add(p);
        p.readyQueued();

        // If idle, activate first process in queue.
        if (idle) {
            activeProcess = cpuQueue.pop();
            idle = false;
            p.leftCpuQueue(clock);
            return p.cpuActive(maxCpuTime, clock);
        }
    ...
    public Event switchProcess(long clock) {
        ...
        if (!cpuQueue.isEmpty()) {
            setActiveProcess(cpuQueue.pop());
            statistics.nofProcessSwitches++;
            activeProcess.leftCpuQueue(clock);
            return activeProcess.cpuActive(maxCpuTime, clock);
        }
        idle = true;
        return null;
    }
